...
class App extends Component {
    ...
    componentDidMount() {
        ...
        let promedioMayor = (array) => {
            ...
            return maxCount / array.length
        }

        d3.text(cancer).then(data => {
            var arreglosIniciales = data.split(">");
            var respuesta = [];
            for (let index = 0; index < arreglosIniciales.length; index++) {
                if (index === 0) {
                    continue;
                }
                var myString = arreglosIniciales[index];
            ...
            for (let index = 0; index < respuesta[0].length; index++) {
                ...
                for (let indexInterno = 0; indexInterno < respuesta.length; indexInterno++) {
                    componentes.push(respuesta[indexInterno][index]);
                }
                var objetoTemp = new myObject(index, promedioMayor(componentes));
